...
- The global favicon is generated by `app/icon.tsx` from the first calligraphic `G` of the existing official Header wordmark, placed on the warm ivory brand background. The `/icon` metadata route is excluded from locale handling in `proxy.ts`. No default Vercel favicon or separate invented brand artwork is used.
- Official partners:
...
- Partners page editorial assets:
  ...
  - FINANCE CONSULTING GROUP uses its logo-only visual: `public/images/partners/finance-consulting-group.png`; no editorial photograph or placeholder is rendered.
  - SEMBRANCHER: `public/images/partners/sembrancherpartner.jpg`
- Partners page direction: MRG and Finance Consulting Group use a centered logo-only right column on the existing background with no card; HEROE'S image reaches the right viewport edge and Sembrancher image reaches the left viewport edge. The global partner strip remains logo-only, and the dedicated `/partners` page is the only place using the remaining editorial partner photographs.
- Partners mobile order is strict per partner: content then visual for HEROE'S and Sembrancher; number/designation, title, logo, description and CTA for the MRG and Finance Consulting Group logo-only chapters. Visit links are text-only with no arrow or icon.
